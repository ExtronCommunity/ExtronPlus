...
public class CommandDontSkipNight implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        if(ExtronPlus.INSTANCE.getSleepSkippingHandler().getInBed().isEmpty()) {
            commandSender.sendMessage("§cNo one is currently sleeping");
        }
        else if(ExtronPlus.INSTANCE.getSleepSkippingHandler().isDoSkipNight()) {
            ...
            for(Player p : ExtronPlus.INSTANCE.getSleepSkippingHandler().getInBed()) {
